...
serve(async (req) => {
  ...
  try {
    ...
    if (!user?.email) {
      throw new Error("User niet geauthenticeerd");
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: "price_1SRJeeRARD4nrneY7ZsVx7dc",
          quantity: 1,
        },
      ],
      mode: "payment",
      success_url: `${req.headers.get("origin")}/scan?payment=success`,
      cancel_url: `${req.headers.get("origin")}/business?payment=cancelled`,
    });

    console.log("Checkout sessie aangemaakt voor:", user.email);
